...
class LoginRepositoryImpl @Inject constructor(
    ...
    override suspend fun signUp(username: String, password: String, nickname: String): Result<String> {
        return try {
            val request = SignUpRequest(username, nickname, password) // 순서 주의
            Log.d("SignUpDebug", "📤 보낸 요청: $request")

            val response = loginService.signUp(request)
            Log.d("SignUpDebug", "📥 받은 응답: ${response.code()}")

            if (response.isSuccessful) {
                val body = response.body()
                if (body != null) {
                    Log.d("SignUpDebug", "✅ 회원가입 성공: ${body.message}")
                    Result.success(body.message)
                } else {
                    Log.e("SignUpDebug", "❗ 응답 바디 없음")
                    Result.failure(Exception("응답 바디 없음"))
                }
            } else {
                val error = response.errorBody()?.string()
                Log.e("SignUpDebug", "❌ 실패 응답: ${response.code()} / $error")
                Result.failure(Exception("회원가입 실패: ${response.code()}"))
            }
        } catch (e: Exception) {
            Log.e("SignUpDebug", "❌ 예외 발생: ${e.message}")
            Result.failure(e)
        }
    }
